...
def parse_image_dataset(root: Union[str, Path], illum: bool = False) -> pl.DataFrame:
    root = Path(root)
    print(f"root: {root}")
        ...
        if plate.is_dir() and plate.name != "Output":
            for path in plate.rglob("*.tif*"):
                ...
                if illum:
                    ...
                    row["PathName_Illum"] = "/workspace_images/Output/Illum_files_MPI"
...
def pivot_df(df: pl.DataFrame, illum: bool=False) -> pl.DataFrame:
...
def prepare_CSVs(path, output, illum:bool=False):
    df = parse_image_dataset(path, illum=illum)
